...
BOS_Status DMA_STREAM_Setup(UART_HandleTypeDef *huartSrc,UART_HandleTypeDef *huartDst,uint16_t num){
	...
	if(dstPort == 0){
		...
		memset(StreamBuffer[GetPort(huartSrc) - 1],0,STREAM_BUF_SIZE);
		if(HAL_OK != HAL_UARTEx_ReceiveToIdle_DMA(huartSrc,StreamBuffer[GetPort(huartSrc) - 1],num))
			return Status =BOS_ERROR;
		__HAL_DMA_DISABLE_IT(hDMA,DMA_IT_HT);
	}
	else{
