...
static const CLI_Command_Definition_t xIPConfig =
{
	...
	{
		( const int8_t * ) "ip-debug-stats", /* The command string to type. */
		( const int8_t * ) "ip-debug-stats:\r\n Shows some IP stack stats useful for debug - an example only.\r\n\r\n",
...
{
	( const int8_t * ) "echo-3-parameters",
	( const int8_t * ) "echo-3-parameters <param1> <param2> <param3>:\r\n Expects three parameters, echos each in turn\r\n\r\n",
...
static const CLI_Command_Definition_t xParameterEcho =
{
	...
	{
		( const int8_t * ) "ping",
		( const int8_t * ) "ping <ipaddress> <optional:bytes to send>:\r\n for example, ping 192.168.0.3 8, or ping www.example.com\r\n\r\n",
...
void vRegisterCLICommands_UDP( void )
{
...
static portBASE_TYPE lParameterNumber = 0;
	...
	else
	{
		...
		strncat( ( char * )pcWriteBuffer, pcParameter, xParameterStringLength );
...
uint32_t ulAddress;
	...
	if( ulAddress != 0 )
	{
		FreeRTOS_inet_ntoa( ulAddress,  &( pcWriteBuffer[ strlen( ( char * )pcWriteBuffer ) ] ) );
	}
...
void UDP_ExampleInit(void)
{
    /* Register CLI commands. */
    vRegisterCLICommands_UDP();
